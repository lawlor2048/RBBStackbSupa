...
import { CustomerType, DebugResponse, FormData } from './types';
...
export default function CoverageForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, debug } = await checkCoverage(formData.eircode);
      setCoverageData(data);
      setDebugResponse(debug);
    } catch (error: any) { // Use any here to handle dynamic debug property
      console.error('Form submission error:', error);
      
      // Always set the debug info if available
      if (error.debug) {
        setDebugResponse(error.debug);
      }
      ...
      setError(
        error instanceof Error 
          ...
          : 'An unexpected error occurred while checking coverage'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {debugResponse && <DebugInfo debug={debugResponse} />}
    </div>
  );
}